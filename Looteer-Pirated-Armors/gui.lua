...
gui.elements = {
   ...
   affix_settings = {
      ...
      -- armors_toggle = checkbox:new(false, get_hash(plugin_label .. "_armors_toggle")),
...
function gui:render()
   ...
   if gui.elements.general.tree:push("General Settings") then
      ...
      if gui.elements.affix_settings.tree:push("Affix Settings") then
         ...
         if gui.elements.affix_settings.innerTree:push("Armors Settings") then
         -- gui.elements.affix_settings.armors_toggle:render("Force settings", "Force settings to override Legendary Ga Count")
